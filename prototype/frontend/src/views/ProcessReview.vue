<template>
  <div class="page process-review">
    <el-card shadow="never">
      <template #header>
        <div class="card-header">
          <span>可制造性分析与优化软件</span>
        </div>
      </template>
      <div class="import-section">
        <div class="section-title">导入工艺输入</div>
        <p class="section-desc">上传工艺文件或粘贴工艺内容，用于后续审查。</p>
        <el-row :gutter="24">
          <el-col :span="12">
      ...
      <div v-if="importedCount > 0" class="result-hint">
        ...
        已导入 {{ importedCount }} 项工艺输入，可在下方进行审查或继续导入。
      </div>
    </el-card>
  </div>
</template>
...
function submitPaste() {
  ...
  ElMessage.success('已添加粘贴内容，可继续导入或开始审查')
  pastedContent.value = ''
}
</script>
